...
plot2 <- ggplot(plot_input, aes(x = reorder(Assay, log2fc), y = Count, colour = time,
...
```{r}
plot3 <- ggplot(plot_input, aes(x = reorder(Assay,log2fc), y = log2fc))+
    ...
    theme(axis.title = element_text(size = 16, colour = "black", face = "bold"))+
    theme(legend.text = element_text(size = 12))+
    xlab("Gene")+ylab("Log2FC expression")+coord_flip()
ggsave(plot = plot3, filename = "top20genes_IM_baseline_24hr._log2fc.barplot.png", dpi = 500,
height = 5, width = 8)
plot3

plot4 <- ggplot(plot_input, aes(x = reorder(Assay,fold_change), y = fold_change))+
    geom_hline(yintercept = 0, linetype = "dashed", colour = "black")+
    ...
    theme(legend.text = element_text(size = 12))+
    xlab("Gene")+ylab("Log2FC expression")+coord_flip()
ggsave(plot = plot4, filename = "top20genes_IM_baseline_24hr_abs_fc.barplot.png", dpi = 500,
height = 5, width = 8)
plot4
```